...
int hardwareInitialize()
{
  ...
  std::string binary_file = getBoardBinaryFile("simd2_manually", theDevices[0]);
  printf("Using AOCX: %s\n", binary_file.c_str());
  ...
  unsigned int* aFrameBuffer)
{
  ...
  for(unsigned i = 0; i < numDevices; rowOffset += rowsPerDevice[i++])
  {
    ...
    theStatus = clSetKernelArg(theKernels[i], 5, sizeof(cl_mem), (void*)&theHardColorTable);
    theStatus = clSetKernelArg(theKernels[i], 6, sizeof(cl_uint), (void*)&theWidth);
