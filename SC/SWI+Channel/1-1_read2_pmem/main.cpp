...
int main(int argc, char **argv) {
    ...
    clStatus = clEnqueueWriteBuffer(
        ocl.clCommandQueue_in, d_in_out, CL_TRUE, 0, p.in_size * sizeof(int),
    ...
    for(int rep = 0; rep < p.n_warmup + p.n_reps; rep++) {
        ...
        clSetKernelArg(ocl.clKernel_in, 0, sizeof(int), &p.in_size);
        clSetKernelArg(ocl.clKernel_in, 1, sizeof(cl_mem), &d_in_out);
        clStatus = clEnqueueTask(ocl.clCommandQueue_in, ocl.clKernel_in, 0, NULL, NULL);
        CL_ERR();
        

		clSetKernelArg(ocl.clKernel_0, 0, sizeof(int), &p.in_size);
        clSetKernelArg(ocl.clKernel_0, 1, sizeof(int), &p.remove_value);
        clSetKernelArg(ocl.clKernel_0, 2, sizeof(cl_mem), &d_in_out);
