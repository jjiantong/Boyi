...
struct OpenCLSetup {
    ...
    OpenCLSetup(int platform, int device) {
		...
		// Create the program.
		std::string binary_file = getBoardBinaryFile("1-1_read2_pmem", clDeviceID);
		printf("\nUsing AOCX:%s\n",binary_file.c_str());
		...
		// Kernel.
        clKernel_in  = clCreateKernel(clProgram, "StreamCompaction_in", &clStatus);
		clKernel_0   = clCreateKernel(clProgram, "StreamCompaction_0", &clStatus);
        CL_ERR();
    }
