...
int main(int argc, char **argv) {
	...
	clStatus = clEnqueueWriteBuffer(
        ocl.clCommandQueue_in, d_histo, CL_TRUE, 0, p.n_bins * sizeof(unsigned int), h_histo, 0, NULL, NULL);	
    ...
    for(int rep = 0; rep < p.n_warmup + p.n_reps; rep++) {
		...
		clSetKernelArg(ocl.clKernel_1, 0, sizeof(unsigned int), &p.in_size);
        clSetKernelArg(ocl.clKernel_1, 1, sizeof(unsigned int), &p.n_bins);
        clStatus = clEnqueueTask(ocl.clCommandQueue_1, ocl.clKernel_1, 0, NULL, NULL);
		CL_ERR();

		clSetKernelArg(ocl.clKernel_2, 0, sizeof(unsigned int), &p.in_size);
        clSetKernelArg(ocl.clKernel_2, 1, sizeof(unsigned int), &p.n_bins);
        clStatus = clEnqueueTask(ocl.clCommandQueue_2, ocl.clKernel_2, 0, NULL, NULL);
		CL_ERR();

		clSetKernelArg(ocl.clKernel_3, 0, sizeof(unsigned int), &p.in_size);
        clSetKernelArg(ocl.clKernel_3, 1, sizeof(unsigned int), &p.n_bins);
        clStatus = clEnqueueTask(ocl.clCommandQueue_3, ocl.clKernel_3, 0, NULL, NULL);
		CL_ERR();
		
		clSetKernelArg(ocl.clKernel_4, 0, sizeof(unsigned int), &p.in_size);
        clSetKernelArg(ocl.clKernel_4, 1, sizeof(unsigned int), &p.n_bins);
        clStatus = clEnqueueTask(ocl.clCommandQueue_4, ocl.clKernel_4, 0, NULL, NULL);
		CL_ERR();

		clSetKernelArg(ocl.clKernel_5, 0, sizeof(unsigned int), &p.in_size);
        clSetKernelArg(ocl.clKernel_5, 1, sizeof(unsigned int), &p.n_bins);
        clStatus = clEnqueueTask(ocl.clCommandQueue_5, ocl.clKernel_5, 0, NULL, NULL);
		CL_ERR();

		clSetKernelArg(ocl.clKernel_6, 0, sizeof(unsigned int), &p.in_size);
        clSetKernelArg(ocl.clKernel_6, 1, sizeof(unsigned int), &p.n_bins);
        clStatus = clEnqueueTask(ocl.clCommandQueue_6, ocl.clKernel_6, 0, NULL, NULL);
		CL_ERR();

		clSetKernelArg(ocl.clKernel_7, 0, sizeof(unsigned int), &p.in_size);
        clSetKernelArg(ocl.clKernel_7, 1, sizeof(unsigned int), &p.n_bins);
        clStatus = clEnqueueTask(ocl.clCommandQueue_7, ocl.clKernel_7, 0, NULL, NULL);
		CL_ERR();

		clSetKernelArg(ocl.clKernel_8, 0, sizeof(unsigned int), &p.in_size);
        clSetKernelArg(ocl.clKernel_8, 1, sizeof(unsigned int), &p.n_bins);
        clStatus = clEnqueueTask(ocl.clCommandQueue_8, ocl.clKernel_8, 0, NULL, NULL);
		CL_ERR();

		clSetKernelArg(ocl.clKernel_9, 0, sizeof(unsigned int), &p.in_size);
        clSetKernelArg(ocl.clKernel_9, 1, sizeof(unsigned int), &p.n_bins);
        clStatus = clEnqueueTask(ocl.clCommandQueue_9, ocl.clKernel_9, 0, NULL, NULL);
		CL_ERR();

		clSetKernelArg(ocl.clKernel_10, 0, sizeof(unsigned int), &p.in_size);
        clSetKernelArg(ocl.clKernel_10, 1, sizeof(unsigned int), &p.n_bins);
        clStatus = clEnqueueTask(ocl.clCommandQueue_10, ocl.clKernel_10, 0, NULL, NULL);
		CL_ERR();

		clSetKernelArg(ocl.clKernel_11, 0, sizeof(unsigned int), &p.in_size);
        clSetKernelArg(ocl.clKernel_11, 1, sizeof(unsigned int), &p.n_bins);
        clStatus = clEnqueueTask(ocl.clCommandQueue_11, ocl.clKernel_11, 0, NULL, NULL);
		CL_ERR();

		clSetKernelArg(ocl.clKernel_12, 0, sizeof(unsigned int), &p.in_size);
        clSetKernelArg(ocl.clKernel_12, 1, sizeof(unsigned int), &p.n_bins);
        clStatus = clEnqueueTask(ocl.clCommandQueue_12, ocl.clKernel_12, 0, NULL, NULL);
		CL_ERR();

		clSetKernelArg(ocl.clKernel_13, 0, sizeof(unsigned int), &p.in_size);
        clSetKernelArg(ocl.clKernel_13, 1, sizeof(unsigned int), &p.n_bins);
        clStatus = clEnqueueTask(ocl.clCommandQueue_13, ocl.clKernel_13, 0, NULL, NULL);
		CL_ERR();

		clSetKernelArg(ocl.clKernel_14, 0, sizeof(unsigned int), &p.in_size);
        clSetKernelArg(ocl.clKernel_14, 1, sizeof(unsigned int), &p.n_bins);
        clStatus = clEnqueueTask(ocl.clCommandQueue_14, ocl.clKernel_14, 0, NULL, NULL);
		CL_ERR();


		
		clSetKernelArg(ocl.clKernel_15, 0, sizeof(unsigned int), &p.in_size);
		clSetKernelArg(ocl.clKernel_15, 1, sizeof(unsigned int), &p.n_bins);
    ...
    clStatus = clEnqueueReadBuffer(
        ocl.clCommandQueue_in, d_histo, CL_TRUE, 0, p.n_bins * sizeof(unsigned int), h_histo_merge, 0, NULL, NULL);
    CL_ERR();
    clFinish(ocl.clCommandQueue_in);
