...
struct OpenCLSetup {
    ...
    cl_command_queue clCommandQueue1;
	cl_command_queue clCommandQueue2;
	cl_command_queue clCommandQueue3;
	cl_command_queue clCommandQueue4;
    ...
    OpenCLSetup(int platform, int device) {
		...
		// Command queue.
        clCommandQueue1 = clCreateCommandQueue(clContext, clDevices[device], 0, &clStatus);
		clCommandQueue2 = clCreateCommandQueue(clContext, clDevices[device], 0, &clStatus);
		clCommandQueue3 = clCreateCommandQueue(clContext, clDevices[device], 0, &clStatus);
		clCommandQueue4 = clCreateCommandQueue(clContext, clDevices[device], 0, &clStatus);
		
        CL_ERR();


		// Create the program.
		std::string binary_file = getBoardBinaryFile("p_inout2_uli3-3_ulo2-1", clDeviceID);
		printf("\nUsing AOCX:%s\n",binary_file.c_str());
    ...
    void release() {
        ...
        clReleaseCommandQueue(clCommandQueue1);
		clReleaseCommandQueue(clCommandQueue2);
		clReleaseCommandQueue(clCommandQueue3);
		clReleaseCommandQueue(clCommandQueue4);
