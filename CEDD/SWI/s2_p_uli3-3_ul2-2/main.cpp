...
int main(int argc, char **argv) {
	...
	h_in_out[FPGA_PROXY] = (unsigned char *)_aligned_malloc(in_size,AOCL_ALIGNMENT);
    ...
    cl_mem         d_interm_fpga_proxy = clCreateBuffer(ocl.clContext, CL_MEM_READ_WRITE, in_size, NULL, &clStatus);
    cl_mem         d_theta_fpga_proxy  = clCreateBuffer(ocl.clContext, CL_MEM_READ_WRITE, in_size, NULL, &clStatus);
    std::atomic<int> next_frame;
    clFinish(ocl.clCommandQueue);
    ALLOC_ERR(h_in_out[CPU_PROXY], h_in_out[FPGA_PROXY], h_interm_cpu_proxy, h_theta_cpu_proxy);
    CL_ERR();
        ...
        proxy_threads.push_back(std::thread([&, proxy_tid]() {
            ...
            if(proxy_tid == FPGA_PROXY) {
				...
				for(int task_id = 0; task_id < n_frames; task_id ++) {
                    ...
                    clSetKernelArg(ocl.clKernel_gauss, 1, sizeof(cl_mem), &d_interm_fpga_proxy);
					//clSetKernelArg(ocl.clKernel_gauss, 2, sizeof(cl_mem), &d_theta_fpga_proxy);
                    clSetKernelArg(ocl.clKernel_gauss, 2, sizeof(int), &colsc);
					clSetKernelArg(ocl.clKernel_gauss, 3, sizeof(int), &rowsc);
                    // Kernel launch
                    clStatus = clEnqueueTask(
                        ocl.clCommandQueue, ocl.clKernel_gauss, 0, NULL, NULL);
                    CL_ERR();
                    ...
                    // SOBEL KERNEL
                    // Set arguments
                    clSetKernelArg(ocl.clKernel_sobel, 0, sizeof(cl_mem), &d_interm_fpga_proxy);
                    clSetKernelArg(ocl.clKernel_sobel, 1, sizeof(cl_mem), &d_in_out);
                    clSetKernelArg(ocl.clKernel_sobel, 2, sizeof(cl_mem), &d_theta_fpga_proxy);
                    clSetKernelArg(ocl.clKernel_sobel, 3, sizeof(int), &colsc);
					clSetKernelArg(ocl.clKernel_sobel, 4, sizeof(int), &rowsc);
                    // Kernel launch
                    clStatus = clEnqueueTask(
                        ocl.clCommandQueue, ocl.clKernel_sobel, 0, NULL, NULL);
                    CL_ERR();				
                    ...
                    clSetKernelArg(ocl.clKernel_nonmax, 1, sizeof(cl_mem), &d_interm_fpga_proxy);
                    clSetKernelArg(ocl.clKernel_nonmax, 2, sizeof(cl_mem), &d_theta_fpga_proxy);
                    clSetKernelArg(ocl.clKernel_nonmax, 3, sizeof(int), &colsc);
					clSetKernelArg(ocl.clKernel_nonmax, 4, sizeof(int), &rowsc);
                    // Kernel launch
                    clStatus = clEnqueueTask(
                        ocl.clCommandQueue, ocl.clKernel_nonmax, 0, NULL, NULL);
                    CL_ERR();
                    ...
                    // HYSTERESIS KERNEL
                    // Set arguments
                    clSetKernelArg(ocl.clKernel_hyst, 0, sizeof(cl_mem), &d_interm_fpga_proxy);
                    clSetKernelArg(ocl.clKernel_hyst, 1, sizeof(cl_mem), &d_in_out);
                    clSetKernelArg(ocl.clKernel_hyst, 2, sizeof(int), &colsc);
					clSetKernelArg(ocl.clKernel_hyst, 3, sizeof(int), &rowsc);
                    // Kernel launch
                    clStatus =
                        clEnqueueTask(ocl.clCommandQueue, ocl.clKernel_hyst, 0, NULL, NULL);
                    CL_ERR();				
					...
					//clStatus = clEnqueueReadBuffer(
                    //    ocl.clCommandQueue, d_interm_fpga_proxy, CL_TRUE, 0, in_size, h_in_out[proxy_tid], 0, NULL, NULL);
                    //CL_ERR();
    ...
    _aligned_free(h_in_out[FPGA_PROXY]);
    ...
    free(all_out_frames);
    clStatus = clReleaseMemObject(d_interm_fpga_proxy);
    clStatus = clReleaseMemObject(d_theta_fpga_proxy);
    CL_ERR();
    ocl.release();
