...
__kernel void gaussian_kernel(	__global unsigned char *data, __global unsigned char *out, __global unsigned char *theta, int cols, int rows) {
	...
	for (int row = 1; row < rows - 1; row ++){
		for (int col = 1; col < cols - 1; col ++){
			...
			for (int i = 0; i < 3; i ++){
				for (int j = 0; j < 3; j ++){
					sumx += sobx[i][j] * out[(i + row - 1) * cols + j + col - 1];
					sumy += soby[i][j] * out[(i + row - 1) * cols + j + col - 1];
				}
			}
			...
			data[pos] =  min(255, max(0, (int)hypot(sumx, sumy)));
	...
	for (int row = 1; row < rows - 1; row ++){
		for (int col = 1; col < cols - 1; col ++){
			...
			unsigned char magnitude = out[pos];
