...
int main(int argc, char **argv) {
    ...
    proxy_threads.push_back(std::thread([&, proxy_tid]() {
		...
		for(int task_id = 0; task_id < n_frames; task_id ++) {
			...
			size_t ls3[2]     = {(size_t)p.n_work_items / 2, (size_t)p.n_work_items / 2};
			size_t gs3[2]     = {(size_t)(colsc - 2) / 2, (size_t)(rowsc - 2) / 2};
			...
			// Kernel launch
			clStatus = clEnqueueNDRangeKernel(
				ocl.clCommandQueue3, ocl.clKernel_nonmax, 2, NULL, gs, ls, 0, NULL, NULL);
			CL_ERR();
			...
			clStatus =
				clEnqueueNDRangeKernel(ocl.clCommandQueue4, ocl.clKernel_hyst, 2, NULL, gs, ls2, 0, NULL, NULL);
			CL_ERR();
