...
__kernel void non_max_supp_kernel(__global unsigned char *data,
    ...
    const int L_SIZE  = 4 * get_local_size(0);
    const int g_row   = get_global_id(1);
    const int g_col_0 = get_global_id(0) * 4 + 0;
	const int g_col_1 = get_global_id(0) * 4 + 1;
    const int g_col_2 = get_global_id(0) * 4 + 2;
	const int g_col_3 = get_global_id(0) * 4 + 3;
    const int l_row   = get_local_id(1) + 1;
    const int l_col_0 = get_local_id(0) * 4 + 0 + 1;
	const int l_col_1 = get_local_id(0) * 4 + 1 + 1;
	const int l_col_2 = get_local_id(0) * 4 + 2 + 1;
	const int l_col_3 = get_local_id(0) * 4 + 3 + 1;

    const int pos_0   = g_row * cols + g_col_0;
	const int pos_1   = g_row * cols + g_col_1;
	const int pos_2   = g_row * cols + g_col_2;
	const int pos_3   = g_row * cols + g_col_3;



    l_data[l_row * (L_SIZE + 2) + l_col_0] = data[pos_0];
	l_data[l_row * (L_SIZE + 2) + l_col_1] = data[pos_1];
	l_data[l_row * (L_SIZE + 2) + l_col_2] = data[pos_2];
	l_data[l_row * (L_SIZE + 2) + l_col_3] = data[pos_3];

    if(l_row == 1) {
        l_data[0 * (L_SIZE + 2) + l_col_0] = data[pos_0 - cols];
		l_data[0 * (L_SIZE + 2) + l_col_1] = data[pos_1 - cols];
		l_data[0 * (L_SIZE + 2) + l_col_2] = data[pos_2 - cols];
		l_data[0 * (L_SIZE + 2) + l_col_3] = data[pos_3 - cols];

        if(l_col_0 == 1)
            l_data[0 * (L_SIZE + 2) + 0] = data[pos_0 - cols - 1];

        else if(l_col_3 == L_SIZE)
            l_data[0 * (L_SIZE + 2) + (L_SIZE + 1)] = data[pos_3 - cols + 1];
    }

    else if(l_row == L_SIZE) {
        l_data[(L_SIZE + 1) * (L_SIZE + 2) + l_col_0] = data[pos_0 + cols];
		l_data[(L_SIZE + 1) * (L_SIZE + 2) + l_col_1] = data[pos_1 + cols];
		l_data[(L_SIZE + 1) * (L_SIZE + 2) + l_col_2] = data[pos_2 + cols];
		l_data[(L_SIZE + 1) * (L_SIZE + 2) + l_col_3] = data[pos_3 + cols];
    ...
    if(l_col_0 == 1)
        ...
        l_data[l_row * (L_SIZE + 2) + (L_SIZE + 1)] = data[pos_3 + 1];
	...
	unsigned char my_magnitude_2 = l_data[l_row * (L_SIZE + 2) + l_col_2];    
	unsigned char my_magnitude_3 = l_data[l_row * (L_SIZE + 2) + l_col_3];
	...
	switch(theta[pos_2]) {
    case 0:
        if(my_magnitude_2 <= l_data[l_row * (L_SIZE + 2) + l_col_2 + 1] || // east
            my_magnitude_2 <= l_data[l_row * (L_SIZE + 2) + l_col_2 - 1]) // west
            p_out_2 = 0;
        else 
    ...
    case 45:
        if(my_magnitude_2 <= l_data[(l_row - 1) * (L_SIZE + 2) + l_col_2 + 1] || // north east
            my_magnitude_2 <= l_data[(l_row + 1) * (L_SIZE + 2) + l_col_2 - 1]) // south west
            p_out_2 = 0;
        else 
            p_out_2 = my_magnitude_2;
        break;
    ...
    case 90:
        if(my_magnitude_2 <= l_data[(l_row - 1) * (L_SIZE + 2) + l_col_2] || // north
            my_magnitude_2 <= l_data[(l_row + 1) * (L_SIZE + 2) + l_col_2]) // south
            p_out_2 = 0;
        else 
            p_out_2 = my_magnitude_2;
        break;
    ...
    case 135:
        if(my_magnitude_2 <= l_data[(l_row - 1) * (L_SIZE + 2) + l_col_2 - 1] || // north west
            my_magnitude_2 <= l_data[(l_row + 1) * (L_SIZE + 2) + l_col_2 + 1]) // south east
            p_out_2 = 0;
        else 
            p_out_2 = my_magnitude_2;
        break;
	...
	switch(theta[pos_3]) {
    case 0:
        if(my_magnitude_3 <= l_data[l_row * (L_SIZE + 2) + l_col_3 + 1] || // east
            my_magnitude_3 <= l_data[l_row * (L_SIZE + 2) + l_col_3 - 1]) // west
            p_out_3 = 0;
        else 
    ...
    case 45:
        if(my_magnitude_3 <= l_data[(l_row - 1) * (L_SIZE + 2) + l_col_3 + 1] || // north east
            my_magnitude_3 <= l_data[(l_row + 1) * (L_SIZE + 2) + l_col_3 - 1]) // south west
            p_out_3 = 0;
        else 
            p_out_3 = my_magnitude_3;
        break;
    ...
    case 90:
        if(my_magnitude_3 <= l_data[(l_row - 1) * (L_SIZE + 2) + l_col_3] || // north
            my_magnitude_3 <= l_data[(l_row + 1) * (L_SIZE + 2) + l_col_3]) // south
            p_out_3 = 0;
        else 
            p_out_3 = my_magnitude_3;
        break;
    ...
    case 135:
        if(my_magnitude_3 <= l_data[(l_row - 1) * (L_SIZE + 2) + l_col_3 - 1] || // north west
            my_magnitude_3 <= l_data[(l_row + 1) * (L_SIZE + 2) + l_col_3 + 1]) // south east
            p_out_3 = 0;
        else 
            p_out_3 = my_magnitude_3;
        break;
