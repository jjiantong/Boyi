...
void RANSAC_model(__global flowvector *restrict flowvectors,
    ...
    params[iter*4+0] = model_param[0];
    params[iter*4+1] = model_param[1];
    params[iter*4+2] = model_param[2];
    params[iter*4+3] = model_param[3];
   
}


__kernel 
void RANSAC_threshold(int flowvector_count, int error_threshold, float convergence_threshold,
    __global flowvector *restrict flowvectors,
    __global int *restrict model_candidate, __global int *restrict outliers_candidate, 
    __global int *g_out_id, __global float *params ) {
    ...
    float model_param[4];
    model_param[0] = params[iter*4+0];
    model_param[1] = params[iter*4+1];
    model_param[2] = params[iter*4+2];
    model_param[3] = params[iter*4+3];
