...
namespace {
  ...
  struct IsSequential : public ModulePass {
    static char ID;
    IsSequential() : ModulePass(ID) {}

    struct AccessP {
      Instruction *index; // index of related gep inst
      int flag;           // case 0: no; case 1: if branch; case 2: for loop
    ...
    bool runOnModule(Module &M) override {
      ...
      // ccnt: number of the pairs of candidates (satisfy R2: same buffer object, output->input)
      ...
      */
      ...
      for(int i = 0; i < ccnt; i++) {
        if(!kkc[i].isSameMAP) {
          errs() << kkc[i].kernel1 << ' ' << kkc[i].arg1 << ' ' << kkc[i].kernel2 << ' ' << kkc[i].arg2 << '\n';
        }
      }
      return false;
    }
...
char IsSequential::ID = 0;
static RegisterPass<IsSequential> X("issequential", "find kernel-to-kernel communication",
