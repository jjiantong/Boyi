...
__kernel void RANSAC_threshold_0(int flowvector_count, int error_threshold,
    ...
    for(int iter = 0; iter < n_tasks; iter++) {
        ...
        if(model_param[0] == -2011){
			...
			int size = flowvector_count / 9;
			#pragma unroll 6
			for(int i = 0; i < size; i++) {
				flowvector fvreg0 = flowvectors[i * 9 + 0]; 
				flowvector fvreg1 = flowvectors[i * 9 + 1]; 
				flowvector fvreg2 = flowvectors[i * 9 + 2]; 
				flowvector fvreg3 = flowvectors[i * 9 + 3]; 
				flowvector fvreg4 = flowvectors[i * 9 + 4]; 
				flowvector fvreg5 = flowvectors[i * 9 + 5]; 
				flowvector fvreg6 = flowvectors[i * 9 + 6]; 
				flowvector fvreg7 = flowvectors[i * 9 + 7]; 
				flowvector fvreg8 = flowvectors[i * 9 + 8]; 
