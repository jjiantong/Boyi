...
int main(int argc, char **argv) {
    ...
    for(int rep = 0; rep < p.n_warmup + p.n_reps; rep++) {
		...
		clSetKernelArg(ocl.clKernel, 3, sizeof(int), &n_fpga_tasks);
		clSetKernelArg(ocl.clKernel, 4, sizeof(cl_mem), &d_flow_vector_array);
		clSetKernelArg(ocl.clKernel, 5, sizeof(cl_mem), &d_random_numbers);
		clSetKernelArg(ocl.clKernel, 6, sizeof(cl_mem), &d_model_candidate);
		clSetKernelArg(ocl.clKernel, 7, sizeof(cl_mem), &d_outliers_candidate);
		clSetKernelArg(ocl.clKernel, 8, sizeof(cl_mem), &d_g_out_id);			
