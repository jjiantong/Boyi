#define _OPENCL_COMPILER_

#define MAX 6000

#include "support/common.h"

// OpenCL baseline kernel ------------------------------------------------------------------------------------------
// Generate model on GPU side
int gen_model_param(int x1, int y1, int vx1, int vy1, int x2, int y2, int vx2, int vy2, float *model_param) {
    float temp;
    temp = (float)((vx1 * (vx1 - (2 * vx2))) + (vx2 * vx2) + (vy1 * vy1) - (vy2 * ((2 * vy1) - vy2)));
    if(temp == 0) { // Check to prevent division by zero
        return (0);
    }
    model_param[0] = (((vx1 * ((-vx2 * x1) + (vx1 * x2) - (vx2 * x2) + (vy2 * y1) - (vy2 * y2))) +
                          (vy1 * ((-vy2 * x1) + (vy1 * x2) - (vy2 * x2) - (vx2 * y1) + (vx2 * y2))) +
                          (x1 * ((vy2 * vy2) + (vx2 * vx2)))) /
                      temp);
    model_param[1] = (((vx2 * ((vy1 * x1) - (vy1 * x2) - (vx1 * y1) + (vx2 * y1) - (vx1 * y2))) +
                          (vy2 * ((-vx1 * x1) + (vx1 * x2) - (vy1 * y1) + (vy2 * y1) - (vy1 * y2))) +
                          (y2 * ((vx1 * vx1) + (vy1 * vy1)))) /
                      temp);

    temp = (float)((x1 * (x1 - (2 * x2))) + (x2 * x2) + (y1 * (y1 - (2 * y2))) + (y2 * y2));
    if(temp == 0) { // Check to prevent division by zero
        return (0);
    }
    model_param[2] = ((((x1 - x2) * (vx1 - vx2)) + ((y1 - y2) * (vy1 - vy2))) / temp);
    model_param[3] = ((((x1 - x2) * (vy1 - vy2)) + ((y2 - y1) * (vx1 - vx2))) / temp);
    return (1);
}


	channel float4 c_model[21];
	channel int c_count[21];

__kernel void RANSAC_model( int n_tasks, __global flowvector *restrict flowvectors,
    __global int *restrict random_numbers) {

	float model_param[4];

    for(int iter = 0; iter < n_tasks; iter++) {

        model_param[0] = 0;
		model_param[1] = 0;
		model_param[2] = 0;
		model_param[3] = 0;

        int        rand_num = random_numbers[iter * 2 + 0];
        flowvector fv[2];
        fv[0]    = flowvectors[rand_num];
        rand_num = random_numbers[iter * 2 + 1];
        fv[1]    = flowvectors[rand_num];

        int ret = 0;
        int vx1 = fv[0].vx - fv[0].x;
        int vy1 = fv[0].vy - fv[0].y;
        int vx2 = fv[1].vx - fv[1].x;
        int vy2 = fv[1].vy - fv[1].y;

        ret = gen_model_param(fv[0].x, fv[0].y, vx1, vy1, fv[1].x, fv[1].y, vx2, vy2, model_param);
        if(ret == 0)
			model_param[0] = -2011;

		float4 model_float4;
		model_float4.s0 = model_param[0];
		model_float4.s1 = model_param[1];
		model_float4.s2 = model_param[2];
		model_float4.s3 = model_param[3];

		write_channel_altera(c_model[0], model_float4);
		write_channel_altera(c_model[1], model_float4);
		write_channel_altera(c_model[2], model_float4);
		write_channel_altera(c_model[3], model_float4);
		write_channel_altera(c_model[4], model_float4);
		write_channel_altera(c_model[5], model_float4);
		write_channel_altera(c_model[6], model_float4);
		write_channel_altera(c_model[7], model_float4);
		write_channel_altera(c_model[8], model_float4);
		write_channel_altera(c_model[9], model_float4);
		write_channel_altera(c_model[10], model_float4);
		write_channel_altera(c_model[11], model_float4);
		write_channel_altera(c_model[12], model_float4);
		write_channel_altera(c_model[13], model_float4);
		write_channel_altera(c_model[14], model_float4);
		write_channel_altera(c_model[15], model_float4);
		write_channel_altera(c_model[16], model_float4);
		write_channel_altera(c_model[17], model_float4);
		write_channel_altera(c_model[18], model_float4);
		write_channel_altera(c_model[19], model_float4);
		write_channel_altera(c_model[20], model_float4);
	}
}


__kernel void RANSAC_threshold_0(int flowvector_count, int error_threshold,
    int n_tasks, __global flowvector *restrict flowvectors) {

    float vx_error, vy_error;
    int   outlier_local_count = 0;

	float model_param[4];

    for(int iter = 0; iter < n_tasks; iter++) {

		float4 model_float4 = read_channel_altera(c_model[0]);
		model_param[0] = model_float4.s0;
		model_param[1] = model_float4.s1;
		model_param[2] = model_float4.s2;
		model_param[3] = model_float4.s3;

        if(model_param[0] == -2011)
            outlier_local_count = MAX;

		else{
			outlier_local_count = 0;

			int cut1 = flowvector_count / 21;
			for(int i = 0; i < cut1; i++) {
				flowvector fvreg = flowvectors[i]; // x, y, vx, vy
				vx_error         = fvreg.x + ((int)((fvreg.x - model_param[0]) * model_param[2]) -
					                     (int)((fvreg.y - model_param[1]) * model_param[3])) -
						fvreg.vx;
				vy_error = fvreg.y + ((int)((fvreg.y - model_param[1]) * model_param[2]) +
					                     (int)((fvreg.x - model_param[0]) * model_param[3])) -
						fvreg.vy;
				if((fabs(vx_error) >= error_threshold) || (fabs(vy_error) >= error_threshold)) {
					outlier_local_count++;
				}
			}
		}

		write_channel_altera(c_count[0], outlier_local_count);
    }
}

__kernel void RANSAC_threshold_1(int flowvector_count, int error_threshold,
    int n_tasks, __global flowvector *restrict flowvectors) {

    float vx_error, vy_error;
    int   outlier_local_count = 0;

	float model_param[4];

    for(int iter = 0; iter < n_tasks; iter++) {

		float4 model_float4 = read_channel_altera(c_model[1]);
		model_param[0] = model_float4.s0;
		model_param[1] = model_float4.s1;
		model_param[2] = model_float4.s2;
		model_param[3] = model_float4.s3;

        if(model_param[0] == -2011)
            outlier_local_count = MAX;

		else{
			outlier_local_count = 0;

			int cut1 = flowvector_count / 21;
			int cut2 = 2 * cut1;
			for(int i = cut1; i < cut2; i++) {
				flowvector fvreg = flowvectors[i]; // x, y, vx, vy
				vx_error         = fvreg.x + ((int)((fvreg.x - model_param[0]) * model_param[2]) -
					                     (int)((fvreg.y - model_param[1]) * model_param[3])) -
						fvreg.vx;
				vy_error = fvreg.y + ((int)((fvreg.y - model_param[1]) * model_param[2]) +
					                     (int)((fvreg.x - model_param[0]) * model_param[3])) -
						fvreg.vy;
				if((fabs(vx_error) >= error_threshold) || (fabs(vy_error) >= error_threshold)) {
					outlier_local_count++;
				}
			}
		}

		write_channel_altera(c_count[1], outlier_local_count);
    }
}


__kernel void RANSAC_threshold_2(int flowvector_count, int error_threshold,
    int n_tasks, __global flowvector *restrict flowvectors) {

    float vx_error, vy_error;
    int   outlier_local_count = 0;

	float model_param[4];

    for(int iter = 0; iter < n_tasks; iter++) {

		float4 model_float4 = read_channel_altera(c_model[2]);
		model_param[0] = model_float4.s0;
		model_param[1] = model_float4.s1;
		model_param[2] = model_float4.s2;
		model_param[3] = model_float4.s3;

        if(model_param[0] == -2011)
            outlier_local_count = MAX;

		else{
			outlier_local_count = 0;

			int cut1 = flowvector_count / 21;
			int cut2 = 2 * cut1;
			int cut3 = 3 * cut1;
			for(int i = cut2; i < cut3; i++) {
				flowvector fvreg = flowvectors[i]; // x, y, vx, vy
				vx_error         = fvreg.x + ((int)((fvreg.x - model_param[0]) * model_param[2]) -
					                     (int)((fvreg.y - model_param[1]) * model_param[3])) -
						fvreg.vx;
				vy_error = fvreg.y + ((int)((fvreg.y - model_param[1]) * model_param[2]) +
					                     (int)((fvreg.x - model_param[0]) * model_param[3])) -
						fvreg.vy;
				if((fabs(vx_error) >= error_threshold) || (fabs(vy_error) >= error_threshold)) {
					outlier_local_count++;
				}
			}
		}

		write_channel_altera(c_count[2], outlier_local_count);
    }
}

__kernel void RANSAC_threshold_3(int flowvector_count, int error_threshold,
    int n_tasks, __global flowvector *restrict flowvectors) {

    float vx_error, vy_error;
    int   outlier_local_count = 0;

	float model_param[4];

    for(int iter = 0; iter < n_tasks; iter++) {

		float4 model_float4 = read_channel_altera(c_model[3]);
		model_param[0] = model_float4.s0;
		model_param[1] = model_float4.s1;
		model_param[2] = model_float4.s2;
		model_param[3] = model_float4.s3;

        if(model_param[0] == -2011)
            outlier_local_count = MAX;

		else{
			outlier_local_count = 0;

			int cut1 = flowvector_count / 21;
			int cut3 = 3 * cut1;
			int cut4 = 4 * cut1;
			for(int i = cut3; i < cut4; i++) {
				flowvector fvreg = flowvectors[i]; // x, y, vx, vy
				vx_error         = fvreg.x + ((int)((fvreg.x - model_param[0]) * model_param[2]) -
					                     (int)((fvreg.y - model_param[1]) * model_param[3])) -
						fvreg.vx;
				vy_error = fvreg.y + ((int)((fvreg.y - model_param[1]) * model_param[2]) +
					                     (int)((fvreg.x - model_param[0]) * model_param[3])) -
						fvreg.vy;
				if((fabs(vx_error) >= error_threshold) || (fabs(vy_error) >= error_threshold)) {
					outlier_local_count++;
				}
			}
		}

		write_channel_altera(c_count[3], outlier_local_count);
    }
}

__kernel void RANSAC_threshold_4(int flowvector_count, int error_threshold,
    int n_tasks, __global flowvector *restrict flowvectors) {

    float vx_error, vy_error;
    int   outlier_local_count = 0;

	float model_param[4];

    for(int iter = 0; iter < n_tasks; iter++) {

		float4 model_float4 = read_channel_altera(c_model[4]);
		model_param[0] = model_float4.s0;
		model_param[1] = model_float4.s1;
		model_param[2] = model_float4.s2;
		model_param[3] = model_float4.s3;

        if(model_param[0] == -2011)
            outlier_local_count = MAX;

		else{
			outlier_local_count = 0;

			int cut1 = flowvector_count / 21;
			int cut4 = 4 * cut1;
			int cut5 = 5 * cut1;
			for(int i = cut4; i < cut5; i++) {
				flowvector fvreg = flowvectors[i]; // x, y, vx, vy
				vx_error         = fvreg.x + ((int)((fvreg.x - model_param[0]) * model_param[2]) -
					                     (int)((fvreg.y - model_param[1]) * model_param[3])) -
						fvreg.vx;
				vy_error = fvreg.y + ((int)((fvreg.y - model_param[1]) * model_param[2]) +
					                     (int)((fvreg.x - model_param[0]) * model_param[3])) -
						fvreg.vy;
				if((fabs(vx_error) >= error_threshold) || (fabs(vy_error) >= error_threshold)) {
					outlier_local_count++;
				}
			}
		}

		write_channel_altera(c_count[4], outlier_local_count);
    }
}

__kernel void RANSAC_threshold_5(int flowvector_count, int error_threshold,
    int n_tasks, __global flowvector *restrict flowvectors) {

    float vx_error, vy_error;
    int   outlier_local_count = 0;

	float model_param[4];

    for(int iter = 0; iter < n_tasks; iter++) {

		float4 model_float4 = read_channel_altera(c_model[5]);
		model_param[0] = model_float4.s0;
		model_param[1] = model_float4.s1;
		model_param[2] = model_float4.s2;
		model_param[3] = model_float4.s3;

        if(model_param[0] == -2011)
            outlier_local_count = MAX;

		else{
			outlier_local_count = 0;

			int cut1 = flowvector_count / 21;
			int cut5 = 5 * cut1;
			int cut6 = 6 * cut1;
			for(int i = cut5; i < cut6; i++) {
				flowvector fvreg = flowvectors[i]; // x, y, vx, vy
				vx_error         = fvreg.x + ((int)((fvreg.x - model_param[0]) * model_param[2]) -
					                     (int)((fvreg.y - model_param[1]) * model_param[3])) -
						fvreg.vx;
				vy_error = fvreg.y + ((int)((fvreg.y - model_param[1]) * model_param[2]) +
					                     (int)((fvreg.x - model_param[0]) * model_param[3])) -
						fvreg.vy;
				if((fabs(vx_error) >= error_threshold) || (fabs(vy_error) >= error_threshold)) {
					outlier_local_count++;
				}
			}
		}

		write_channel_altera(c_count[5], outlier_local_count);
    }
}

__kernel void RANSAC_threshold_6(int flowvector_count, int error_threshold,
    int n_tasks, __global flowvector *restrict flowvectors) {

    float vx_error, vy_error;
    int   outlier_local_count = 0;

	float model_param[4];

    for(int iter = 0; iter < n_tasks; iter++) {

		float4 model_float4 = read_channel_altera(c_model[6]);
		model_param[0] = model_float4.s0;
		model_param[1] = model_float4.s1;
		model_param[2] = model_float4.s2;
		model_param[3] = model_float4.s3;

        if(model_param[0] == -2011)
            outlier_local_count = MAX;

		else{
			outlier_local_count = 0;

			int cut1 = flowvector_count / 21;
			int cut6 = 6 * cut1;
			int cut7 = 7 * cut1;
			for(int i = cut6; i < cut7; i++) {
				flowvector fvreg = flowvectors[i]; // x, y, vx, vy
				vx_error         = fvreg.x + ((int)((fvreg.x - model_param[0]) * model_param[2]) -
					                     (int)((fvreg.y - model_param[1]) * model_param[3])) -
						fvreg.vx;
				vy_error = fvreg.y + ((int)((fvreg.y - model_param[1]) * model_param[2]) +
					                     (int)((fvreg.x - model_param[0]) * model_param[3])) -
						fvreg.vy;
				if((fabs(vx_error) >= error_threshold) || (fabs(vy_error) >= error_threshold)) {
					outlier_local_count++;
				}
			}
		}

		write_channel_altera(c_count[6], outlier_local_count);
    }
}

__kernel void RANSAC_threshold_7(int flowvector_count, int error_threshold,
    int n_tasks, __global flowvector *restrict flowvectors) {

    float vx_error, vy_error;
    int   outlier_local_count = 0;

	float model_param[4];

    for(int iter = 0; iter < n_tasks; iter++) {

		float4 model_float4 = read_channel_altera(c_model[7]);
		model_param[0] = model_float4.s0;
		model_param[1] = model_float4.s1;
		model_param[2] = model_float4.s2;
		model_param[3] = model_float4.s3;

        if(model_param[0] == -2011)
            outlier_local_count = MAX;

		else{
			outlier_local_count = 0;

			int cut1 = flowvector_count / 21;
			int cut7 = 7 * cut1;
			int cut8 = 8 * cut1;
			for(int i = cut7; i < cut8; i++) {
				flowvector fvreg = flowvectors[i]; // x, y, vx, vy
				vx_error         = fvreg.x + ((int)((fvreg.x - model_param[0]) * model_param[2]) -
					                     (int)((fvreg.y - model_param[1]) * model_param[3])) -
						fvreg.vx;
				vy_error = fvreg.y + ((int)((fvreg.y - model_param[1]) * model_param[2]) +
					                     (int)((fvreg.x - model_param[0]) * model_param[3])) -
						fvreg.vy;
				if((fabs(vx_error) >= error_threshold) || (fabs(vy_error) >= error_threshold)) {
					outlier_local_count++;
				}
			}
		}

		write_channel_altera(c_count[7], outlier_local_count);
    }
}

__kernel void RANSAC_threshold_8(int flowvector_count, int error_threshold,
    int n_tasks, __global flowvector *restrict flowvectors) {

    float vx_error, vy_error;
    int   outlier_local_count = 0;

	float model_param[4];

    for(int iter = 0; iter < n_tasks; iter++) {

		float4 model_float4 = read_channel_altera(c_model[8]);
		model_param[0] = model_float4.s0;
		model_param[1] = model_float4.s1;
		model_param[2] = model_float4.s2;
		model_param[3] = model_float4.s3;

        if(model_param[0] == -2011)
            outlier_local_count = MAX;

		else{
			outlier_local_count = 0;

			int cut1 = flowvector_count / 21;
			int cut8 = 8 * cut1;
			int cut9 = 9 * cut1;
			for(int i = cut8; i < cut9; i++) {
				flowvector fvreg = flowvectors[i]; // x, y, vx, vy
				vx_error         = fvreg.x + ((int)((fvreg.x - model_param[0]) * model_param[2]) -
					                     (int)((fvreg.y - model_param[1]) * model_param[3])) -
						fvreg.vx;
				vy_error = fvreg.y + ((int)((fvreg.y - model_param[1]) * model_param[2]) +
					                     (int)((fvreg.x - model_param[0]) * model_param[3])) -
						fvreg.vy;
				if((fabs(vx_error) >= error_threshold) || (fabs(vy_error) >= error_threshold)) {
					outlier_local_count++;
				}
			}
		}

		write_channel_altera(c_count[8], outlier_local_count);
    }
}

__kernel void RANSAC_threshold_9(int flowvector_count, int error_threshold,
    int n_tasks, __global flowvector *restrict flowvectors) {

    float vx_error, vy_error;
    int   outlier_local_count = 0;

	float model_param[4];

    for(int iter = 0; iter < n_tasks; iter++) {

		float4 model_float4 = read_channel_altera(c_model[9]);
		model_param[0] = model_float4.s0;
		model_param[1] = model_float4.s1;
		model_param[2] = model_float4.s2;
		model_param[3] = model_float4.s3;

        if(model_param[0] == -2011)
            outlier_local_count = MAX;

		else{
			outlier_local_count = 0;

			int cut1  = flowvector_count / 21;
			int cut9  = 9 * cut1;
			int cut10 = 10 * cut1;
			for(int i = cut9; i < cut10; i++) {
				flowvector fvreg = flowvectors[i]; // x, y, vx, vy
				vx_error         = fvreg.x + ((int)((fvreg.x - model_param[0]) * model_param[2]) -
					                     (int)((fvreg.y - model_param[1]) * model_param[3])) -
						fvreg.vx;
				vy_error = fvreg.y + ((int)((fvreg.y - model_param[1]) * model_param[2]) +
					                     (int)((fvreg.x - model_param[0]) * model_param[3])) -
						fvreg.vy;
				if((fabs(vx_error) >= error_threshold) || (fabs(vy_error) >= error_threshold)) {
					outlier_local_count++;
				}
			}
		}

		write_channel_altera(c_count[9], outlier_local_count);
    }
}

__kernel void RANSAC_threshold_10(int flowvector_count, int error_threshold,
    int n_tasks, __global flowvector *restrict flowvectors) {

    float vx_error, vy_error;
    int   outlier_local_count = 0;

	float model_param[4];

    for(int iter = 0; iter < n_tasks; iter++) {

		float4 model_float4 = read_channel_altera(c_model[10]);
		model_param[0] = model_float4.s0;
		model_param[1] = model_float4.s1;
		model_param[2] = model_float4.s2;
		model_param[3] = model_float4.s3;

        if(model_param[0] == -2011)
            outlier_local_count = MAX;

		else{
			outlier_local_count = 0;

			int cut1  = flowvector_count / 21;
			int cut10 = 10 * cut1;
			int cut11 = 11 * cut1;
			for(int i = cut10; i < cut11; i++) {
				flowvector fvreg = flowvectors[i]; // x, y, vx, vy
				vx_error         = fvreg.x + ((int)((fvreg.x - model_param[0]) * model_param[2]) -
					                     (int)((fvreg.y - model_param[1]) * model_param[3])) -
						fvreg.vx;
				vy_error = fvreg.y + ((int)((fvreg.y - model_param[1]) * model_param[2]) +
					                     (int)((fvreg.x - model_param[0]) * model_param[3])) -
						fvreg.vy;
				if((fabs(vx_error) >= error_threshold) || (fabs(vy_error) >= error_threshold)) {
					outlier_local_count++;
				}
			}
		}

		write_channel_altera(c_count[10], outlier_local_count);
    }
}

__kernel void RANSAC_threshold_11(int flowvector_count, int error_threshold,
    int n_tasks, __global flowvector *restrict flowvectors) {

    float vx_error, vy_error;
    int   outlier_local_count = 0;

	float model_param[4];

    for(int iter = 0; iter < n_tasks; iter++) {

		float4 model_float4 = read_channel_altera(c_model[11]);
		model_param[0] = model_float4.s0;
		model_param[1] = model_float4.s1;
		model_param[2] = model_float4.s2;
		model_param[3] = model_float4.s3;

        if(model_param[0] == -2011)
            outlier_local_count = MAX;

		else{
			outlier_local_count = 0;

			int cut1  = flowvector_count / 21;
			int cut11 = 11 * cut1;
			int cut12 = 12 * cut1;
			for(int i = cut11; i < cut12; i++) {
				flowvector fvreg = flowvectors[i]; // x, y, vx, vy
				vx_error         = fvreg.x + ((int)((fvreg.x - model_param[0]) * model_param[2]) -
					                     (int)((fvreg.y - model_param[1]) * model_param[3])) -
						fvreg.vx;
				vy_error = fvreg.y + ((int)((fvreg.y - model_param[1]) * model_param[2]) +
					                     (int)((fvreg.x - model_param[0]) * model_param[3])) -
						fvreg.vy;
				if((fabs(vx_error) >= error_threshold) || (fabs(vy_error) >= error_threshold)) {
					outlier_local_count++;
				}
			}
		}

		write_channel_altera(c_count[11], outlier_local_count);
    }
}

__kernel void RANSAC_threshold_12(int flowvector_count, int error_threshold,
    int n_tasks, __global flowvector *restrict flowvectors) {

    float vx_error, vy_error;
    int   outlier_local_count = 0;

	float model_param[4];

    for(int iter = 0; iter < n_tasks; iter++) {

		float4 model_float4 = read_channel_altera(c_model[12]);
		model_param[0] = model_float4.s0;
		model_param[1] = model_float4.s1;
		model_param[2] = model_float4.s2;
		model_param[3] = model_float4.s3;

        if(model_param[0] == -2011)
            outlier_local_count = MAX;

		else{
			outlier_local_count = 0;

			int cut1  = flowvector_count / 21;
			int cut12 = 12 * cut1;
			int cut13 = 13 * cut1;
			for(int i = cut12; i < cut13; i++) {
				flowvector fvreg = flowvectors[i]; // x, y, vx, vy
				vx_error         = fvreg.x + ((int)((fvreg.x - model_param[0]) * model_param[2]) -
					                     (int)((fvreg.y - model_param[1]) * model_param[3])) -
						fvreg.vx;
				vy_error = fvreg.y + ((int)((fvreg.y - model_param[1]) * model_param[2]) +
					                     (int)((fvreg.x - model_param[0]) * model_param[3])) -
						fvreg.vy;
				if((fabs(vx_error) >= error_threshold) || (fabs(vy_error) >= error_threshold)) {
					outlier_local_count++;
				}
			}
		}

		write_channel_altera(c_count[12], outlier_local_count);
    }
}

__kernel void RANSAC_threshold_13(int flowvector_count, int error_threshold,
    int n_tasks, __global flowvector *restrict flowvectors) {

    float vx_error, vy_error;
    int   outlier_local_count = 0;

	float model_param[4];

    for(int iter = 0; iter < n_tasks; iter++) {

		float4 model_float4 = read_channel_altera(c_model[13]);
		model_param[0] = model_float4.s0;
		model_param[1] = model_float4.s1;
		model_param[2] = model_float4.s2;
		model_param[3] = model_float4.s3;

        if(model_param[0] == -2011)
            outlier_local_count = MAX;

		else{
			outlier_local_count = 0;

			int cut1  = flowvector_count / 21;
			int cut13 = 13 * cut1;
			int cut14 = 14 * cut1;
			for(int i = cut13; i < cut14; i++) {
				flowvector fvreg = flowvectors[i]; // x, y, vx, vy
				vx_error         = fvreg.x + ((int)((fvreg.x - model_param[0]) * model_param[2]) -
					                     (int)((fvreg.y - model_param[1]) * model_param[3])) -
						fvreg.vx;
				vy_error = fvreg.y + ((int)((fvreg.y - model_param[1]) * model_param[2]) +
					                     (int)((fvreg.x - model_param[0]) * model_param[3])) -
						fvreg.vy;
				if((fabs(vx_error) >= error_threshold) || (fabs(vy_error) >= error_threshold)) {
					outlier_local_count++;
				}
			}
		}

		write_channel_altera(c_count[13], outlier_local_count);
    }
}

__kernel void RANSAC_threshold_14(int flowvector_count, int error_threshold,
    int n_tasks, __global flowvector *restrict flowvectors) {

    float vx_error, vy_error;
    int   outlier_local_count = 0;

	float model_param[4];

    for(int iter = 0; iter < n_tasks; iter++) {

		float4 model_float4 = read_channel_altera(c_model[14]);
		model_param[0] = model_float4.s0;
		model_param[1] = model_float4.s1;
		model_param[2] = model_float4.s2;
		model_param[3] = model_float4.s3;

        if(model_param[0] == -2011)
            outlier_local_count = MAX;

		else{
			outlier_local_count = 0;

			int cut1  = flowvector_count / 21;
			int cut14 = 14 * cut1;
			int cut15 = 15 * cut1;
			for(int i = cut14; i < cut15; i++) {
				flowvector fvreg = flowvectors[i]; // x, y, vx, vy
				vx_error         = fvreg.x + ((int)((fvreg.x - model_param[0]) * model_param[2]) -
					                     (int)((fvreg.y - model_param[1]) * model_param[3])) -
						fvreg.vx;
				vy_error = fvreg.y + ((int)((fvreg.y - model_param[1]) * model_param[2]) +
					                     (int)((fvreg.x - model_param[0]) * model_param[3])) -
						fvreg.vy;
				if((fabs(vx_error) >= error_threshold) || (fabs(vy_error) >= error_threshold)) {
					outlier_local_count++;
				}
			}
		}

		write_channel_altera(c_count[14], outlier_local_count);
    }
}

__kernel void RANSAC_threshold_15(int flowvector_count, int error_threshold,
    int n_tasks, __global flowvector *restrict flowvectors) {

    float vx_error, vy_error;
    int   outlier_local_count = 0;

	float model_param[4];

    for(int iter = 0; iter < n_tasks; iter++) {

		float4 model_float4 = read_channel_altera(c_model[15]);
		model_param[0] = model_float4.s0;
		model_param[1] = model_float4.s1;
		model_param[2] = model_float4.s2;
		model_param[3] = model_float4.s3;

        if(model_param[0] == -2011)
            outlier_local_count = MAX;

		else{
			outlier_local_count = 0;

			int cut1  = flowvector_count / 21;
			int cut15 = 15 * cut1;
			int cut16 = 16 * cut1;
			for(int i = cut15; i < cut16; i++) {
				flowvector fvreg = flowvectors[i]; // x, y, vx, vy
				vx_error         = fvreg.x + ((int)((fvreg.x - model_param[0]) * model_param[2]) -
					                     (int)((fvreg.y - model_param[1]) * model_param[3])) -
						fvreg.vx;
				vy_error = fvreg.y + ((int)((fvreg.y - model_param[1]) * model_param[2]) +
					                     (int)((fvreg.x - model_param[0]) * model_param[3])) -
						fvreg.vy;
				if((fabs(vx_error) >= error_threshold) || (fabs(vy_error) >= error_threshold)) {
					outlier_local_count++;
				}
			}
		}

		write_channel_altera(c_count[15], outlier_local_count);
    }
}

__kernel void RANSAC_threshold_16(int flowvector_count, int error_threshold,
    int n_tasks, __global flowvector *restrict flowvectors) {

    float vx_error, vy_error;
    int   outlier_local_count = 0;

	float model_param[4];

    for(int iter = 0; iter < n_tasks; iter++) {

		float4 model_float4 = read_channel_altera(c_model[16]);
		model_param[0] = model_float4.s0;
		model_param[1] = model_float4.s1;
		model_param[2] = model_float4.s2;
		model_param[3] = model_float4.s3;

        if(model_param[0] == -2011)
            outlier_local_count = MAX;

		else{
			outlier_local_count = 0;

			int cut1  = flowvector_count / 21;
			int cut16 = 16 * cut1;
			int cut17 = 17 * cut1;
			for(int i = cut16; i < cut17; i++) {
				flowvector fvreg = flowvectors[i]; // x, y, vx, vy
				vx_error         = fvreg.x + ((int)((fvreg.x - model_param[0]) * model_param[2]) -
					                     (int)((fvreg.y - model_param[1]) * model_param[3])) -
						fvreg.vx;
				vy_error = fvreg.y + ((int)((fvreg.y - model_param[1]) * model_param[2]) +
					                     (int)((fvreg.x - model_param[0]) * model_param[3])) -
						fvreg.vy;
				if((fabs(vx_error) >= error_threshold) || (fabs(vy_error) >= error_threshold)) {
					outlier_local_count++;
				}
			}
		}

		write_channel_altera(c_count[16], outlier_local_count);
    }
}

__kernel void RANSAC_threshold_17(int flowvector_count, int error_threshold,
    int n_tasks, __global flowvector *restrict flowvectors) {

    float vx_error, vy_error;
    int   outlier_local_count = 0;

	float model_param[4];

    for(int iter = 0; iter < n_tasks; iter++) {

		float4 model_float4 = read_channel_altera(c_model[17]);
		model_param[0] = model_float4.s0;
		model_param[1] = model_float4.s1;
		model_param[2] = model_float4.s2;
		model_param[3] = model_float4.s3;

        if(model_param[0] == -2011)
            outlier_local_count = MAX;

		else{
			outlier_local_count = 0;

			int cut1  = flowvector_count / 21;
			int cut17 = 17 * cut1;
			int cut18 = 18 * cut1;
			for(int i = cut17; i < cut18; i++) {
				flowvector fvreg = flowvectors[i]; // x, y, vx, vy
				vx_error         = fvreg.x + ((int)((fvreg.x - model_param[0]) * model_param[2]) -
					                     (int)((fvreg.y - model_param[1]) * model_param[3])) -
						fvreg.vx;
				vy_error = fvreg.y + ((int)((fvreg.y - model_param[1]) * model_param[2]) +
					                     (int)((fvreg.x - model_param[0]) * model_param[3])) -
						fvreg.vy;
				if((fabs(vx_error) >= error_threshold) || (fabs(vy_error) >= error_threshold)) {
					outlier_local_count++;
				}
			}
		}

		write_channel_altera(c_count[17], outlier_local_count);
    }
}

__kernel void RANSAC_threshold_18(int flowvector_count, int error_threshold,
    int n_tasks, __global flowvector *restrict flowvectors) {

    float vx_error, vy_error;
    int   outlier_local_count = 0;

	float model_param[4];

    for(int iter = 0; iter < n_tasks; iter++) {

		float4 model_float4 = read_channel_altera(c_model[18]);
		model_param[0] = model_float4.s0;
		model_param[1] = model_float4.s1;
		model_param[2] = model_float4.s2;
		model_param[3] = model_float4.s3;

        if(model_param[0] == -2011)
            outlier_local_count = MAX;

		else{
			outlier_local_count = 0;

			int cut1  = flowvector_count / 21;
			int cut18 = 18 * cut1;
			int cut19 = 19 * cut1;
			for(int i = cut18; i < cut19; i++) {
				flowvector fvreg = flowvectors[i]; // x, y, vx, vy
				vx_error         = fvreg.x + ((int)((fvreg.x - model_param[0]) * model_param[2]) -
					                     (int)((fvreg.y - model_param[1]) * model_param[3])) -
						fvreg.vx;
				vy_error = fvreg.y + ((int)((fvreg.y - model_param[1]) * model_param[2]) +
					                     (int)((fvreg.x - model_param[0]) * model_param[3])) -
						fvreg.vy;
				if((fabs(vx_error) >= error_threshold) || (fabs(vy_error) >= error_threshold)) {
					outlier_local_count++;
				}
			}
		}

		write_channel_altera(c_count[18], outlier_local_count);
    }
}

__kernel void RANSAC_threshold_19(int flowvector_count, int error_threshold,
    int n_tasks, __global flowvector *restrict flowvectors) {

    float vx_error, vy_error;
    int   outlier_local_count = 0;

	float model_param[4];

    for(int iter = 0; iter < n_tasks; iter++) {

		float4 model_float4 = read_channel_altera(c_model[19]);
		model_param[0] = model_float4.s0;
		model_param[1] = model_float4.s1;
		model_param[2] = model_float4.s2;
		model_param[3] = model_float4.s3;

        if(model_param[0] == -2011)
            outlier_local_count = MAX;

		else{
			outlier_local_count = 0;

			int cut1  = flowvector_count / 21;
			int cut19 = 19 * cut1;
			int cut20 = 20 * cut1;
			for(int i = cut19; i < cut20; i++) {
				flowvector fvreg = flowvectors[i]; // x, y, vx, vy
				vx_error         = fvreg.x + ((int)((fvreg.x - model_param[0]) * model_param[2]) -
					                     (int)((fvreg.y - model_param[1]) * model_param[3])) -
						fvreg.vx;
				vy_error = fvreg.y + ((int)((fvreg.y - model_param[1]) * model_param[2]) +
					                     (int)((fvreg.x - model_param[0]) * model_param[3])) -
						fvreg.vy;
				if((fabs(vx_error) >= error_threshold) || (fabs(vy_error) >= error_threshold)) {
					outlier_local_count++;
				}
			}
		}

		write_channel_altera(c_count[19], outlier_local_count);
    }
}



__kernel void RANSAC_threshold_20(int flowvector_count, int error_threshold,
    int n_tasks, __global flowvector *restrict flowvectors) {

    float vx_error, vy_error;
    int   outlier_local_count = 0;

	float model_param[4];

    for(int iter = 0; iter < n_tasks; iter++) {

		float4 model_float4 = read_channel_altera(c_model[20]);
		model_param[0] = model_float4.s0;
		model_param[1] = model_float4.s1;
		model_param[2] = model_float4.s2;
		model_param[3] = model_float4.s3;

        if(model_param[0] == -2011)
            outlier_local_count = MAX;

		else{
			outlier_local_count = 0;

			int cut1  = flowvector_count / 21;
			int cut20 = 20 * cut1;
			for(int i = cut20; i < flowvector_count; i++) {
				flowvector fvreg = flowvectors[i]; // x, y, vx, vy
				vx_error         = fvreg.x + ((int)((fvreg.x - model_param[0]) * model_param[2]) -
					                     (int)((fvreg.y - model_param[1]) * model_param[3])) -
						fvreg.vx;
				vy_error = fvreg.y + ((int)((fvreg.y - model_param[1]) * model_param[2]) +
					                     (int)((fvreg.x - model_param[0]) * model_param[3])) -
						fvreg.vy;
				if((fabs(vx_error) >= error_threshold) || (fabs(vy_error) >= error_threshold)) {
					outlier_local_count++;
				}
			}
		}

		write_channel_altera(c_count[20], outlier_local_count);
    }
}


__kernel void RANSAC_out(int flowvector_count, float convergence_threshold,
    int n_tasks,
    __global int *restrict model_candidate, __global int *restrict outliers_candidate, 
    __global int *g_out_id ) {

	int index = 0;
    for(int iter = 0; iter < n_tasks; iter++) {

		int outlier_count_0  = read_channel_altera(c_count[0]);
		int outlier_count_1  = read_channel_altera(c_count[1]);
		int outlier_count_2  = read_channel_altera(c_count[2]);
		int outlier_count_3  = read_channel_altera(c_count[3]);
		int outlier_count_4  = read_channel_altera(c_count[4]);
		int outlier_count_5  = read_channel_altera(c_count[5]);
		int outlier_count_6  = read_channel_altera(c_count[6]);
		int outlier_count_7  = read_channel_altera(c_count[7]);
		int outlier_count_8  = read_channel_altera(c_count[8]);	
		int outlier_count_9  = read_channel_altera(c_count[9]);
		int outlier_count_10 = read_channel_altera(c_count[10]);
		int outlier_count_11 = read_channel_altera(c_count[11]);
		int outlier_count_12 = read_channel_altera(c_count[12]);
		int outlier_count_13 = read_channel_altera(c_count[13]);
		int outlier_count_14 = read_channel_altera(c_count[14]);
		int outlier_count_15 = read_channel_altera(c_count[15]);	
		int outlier_count_16 = read_channel_altera(c_count[16]);
		int outlier_count_17 = read_channel_altera(c_count[17]);
		int outlier_count_18 = read_channel_altera(c_count[18]);
		int outlier_count_19 = read_channel_altera(c_count[19]);
		int outlier_count_20 = read_channel_altera(c_count[20]);			
		int outlier_count    = outlier_count_0 + outlier_count_1 + outlier_count_2 
								+ outlier_count_3 + outlier_count_4 + outlier_count_5 
								+ outlier_count_6 + outlier_count_7 + outlier_count_8
								+ outlier_count_9 + outlier_count_10 + outlier_count_11
								+ outlier_count_12 + outlier_count_13 + outlier_count_14
								+ outlier_count_15 + outlier_count_16 + outlier_count_17
								+ outlier_count_18 + outlier_count_19 + outlier_count_20;

		if(outlier_count < flowvector_count * convergence_threshold) {
			outliers_candidate[index] = outlier_count;
			model_candidate[index] = iter;
			index ++;
        }      
    }
	g_out_id[0] = index;
}
