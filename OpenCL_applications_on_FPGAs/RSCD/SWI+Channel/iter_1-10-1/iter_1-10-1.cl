...
__kernel void RANSAC_threshold_0(int flowvector_count, int error_threshold,
    ...
    for(int iter = 0; iter < n_tasks; iter += 10) {
...
__kernel void RANSAC_threshold_1(int flowvector_count, int error_threshold,
    ...
    for(int iter = 1; iter < n_tasks; iter += 10) {
...
__kernel void RANSAC_threshold_2(int flowvector_count, int error_threshold,
    ...
    for(int iter = 2; iter < n_tasks; iter += 10) {
...
__kernel void RANSAC_threshold_3(int flowvector_count, int error_threshold,
    ...
    for(int iter = 3; iter < n_tasks; iter += 10) {
...
__kernel void RANSAC_threshold_4(int flowvector_count, int error_threshold,
    ...
    for(int iter = 4; iter < n_tasks; iter += 10) {
...
__kernel void RANSAC_threshold_5(int flowvector_count, int error_threshold,
    ...
    for(int iter = 5; iter < n_tasks; iter += 10) {
...
__kernel void RANSAC_threshold_6(int flowvector_count, int error_threshold,
    ...
    for(int iter = 6; iter < n_tasks; iter += 10) {
...
__kernel void RANSAC_threshold_7(int flowvector_count, int error_threshold,
    ...
    for(int iter = 7; iter < n_tasks; iter += 10) {
...
__kernel void RANSAC_threshold_8(int flowvector_count, int error_threshold,
    ...
    for(int iter = 8; iter < n_tasks; iter += 10) {
...
__kernel void RANSAC_threshold_9(int flowvector_count, int error_threshold,
    ...
    for(int iter = 9; iter < n_tasks; iter += 10) {
...
__kernel void RANSAC_out(int flowvector_count, float convergence_threshold,
    ...
    for(int iter = 0; iter < n_tasks; iter += 10) {
		...
		if(outlier_count_4 < flowvector_count * convergence_threshold) {
			outliers_candidate[index] = outlier_count_4;
			model_candidate[index] = iter + 4;
			index ++;
        }    

		if(outlier_count_5 < flowvector_count * convergence_threshold) {
			outliers_candidate[index] = outlier_count_5;
			model_candidate[index] = iter + 5;
			index ++;
        }  

		if(outlier_count_6 < flowvector_count * convergence_threshold) {
			outliers_candidate[index] = outlier_count_6;
			model_candidate[index] = iter + 6;
			index ++;
        }    

		if(outlier_count_7 < flowvector_count * convergence_threshold) {
			outliers_candidate[index] = outlier_count_7;
			model_candidate[index] = iter + 7;
			index ++;
        }        
      
		if(outlier_count_8 < flowvector_count * convergence_threshold) {
			outliers_candidate[index] = outlier_count_8;
			model_candidate[index] = iter + 8;
			index ++;
        }    

		if(outlier_count_9 < flowvector_count * convergence_threshold) {
			outliers_candidate[index] = outlier_count_9;
			model_candidate[index] = iter + 9;
			index ++;
        }  
