...
struct OpenCLSetup {
    ...
    OpenCLSetup(int platform, int device) {
		...
		binary_file = getBoardBinaryFile("baseline_2k", clDeviceID);
		printf("\nUsing AOCX:%s\n",binary_file.c_str());
		...
		// Kernel.
		clKernel  = clCreateKernel(clProgram, "RANSAC_model", &clStatus);
		clKernel2 = clCreateKernel(clProgram, "RANSAC_threshold", &clStatus);
        CL_ERR();
    }
