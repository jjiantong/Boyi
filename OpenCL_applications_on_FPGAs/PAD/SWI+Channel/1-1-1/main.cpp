...
int main(int argc, char **argv) {
    ...
    for(int rep = 0; rep < p.n_warmup + p.n_reps; rep++) {
        ...
        clSetKernelArg(ocl.clKernel_out, 0, sizeof(int), &p.n);
        clSetKernelArg(ocl.clKernel_out, 1, sizeof(int), &p.m);
        clSetKernelArg(ocl.clKernel_out, 2, sizeof(int), &p.pad);
        clSetKernelArg(ocl.clKernel_out, 3, sizeof(cl_mem), &d_in_out);
