...
struct OpenCLSetup {
    ...
    OpenCLSetup(int platform, int device) {
		...
		// Create the program.
		std::string binary_file = getBoardBinaryFile("1-1-1", clDeviceID);
		printf("\nUsing AOCX:%s\n",binary_file.c_str());
		...
		// Kernel.
        clKernel_in  = clCreateKernel(clProgram, "Padding_in", &clStatus);
		clKernel_0   = clCreateKernel(clProgram, "Padding_0", &clStatus);
		clKernel_out = clCreateKernel(clProgram, "Padding_out", &clStatus);
        CL_ERR();
    }
