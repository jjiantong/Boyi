...
int main(int argc, char **argv) {
    ...
    clStatus = clEnqueueWriteBuffer(
        ocl.clCommandQueue, d_in_out, CL_TRUE, 0, in_size * sizeof(Taa), h_in_out, 0, NULL, 
        NULL);
    clFinish(ocl.clCommandQueue);
    CL_ERR();
    ...
    for(int rep = 0; rep < p.n_warmup + p.n_reps; rep++) {
        ...
        clStatus = clEnqueueWriteBuffer(
            ocl.clCommandQueue, d_in_out, CL_TRUE, 0, in_size * sizeof(Taa), h_in_out, 0, 
            NULL, NULL);
        CL_ERR();
        clFinish(ocl.clCommandQueue);
        ...
        clSetKernelArg(ocl.clKernel, 3, sizeof(cl_mem), &d_in_out);
		//clSetKernelArg(ocl.clKernel, 4, sizeof(cl_mem), &d_in_out);
