...
int main(int argc, char **argv) {
    ...
    for(int rep = 0; rep < p.n_reps + p.n_warmup; rep++) {
        ...
        for(int n_consumed_tasks = 0; n_consumed_tasks < p.pool_size; n_consumed_tasks += p.queue_size) {
			...
			clSetKernelArg(ocl.clKernel_7, 0, sizeof(int *), &d_histo_queues);
            clSetKernelArg(ocl.clKernel_7, 1, sizeof(int), &p.queue_size);
			clSetKernelArg(ocl.clKernel_7, 2, sizeof(int), &frame_size);
            clSetKernelArg(ocl.clKernel_7, 3, sizeof(int), &p.n_bins);
