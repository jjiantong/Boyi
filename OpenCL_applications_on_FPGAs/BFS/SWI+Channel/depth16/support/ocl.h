...
struct OpenCLSetup {
    ...
    OpenCLSetup(int platform, int device) {
		...
		// Create the program.
		std::string binary_file = getBoardBinaryFile("depth16", clDeviceID);
		printf("\nUsing AOCX:%s\n",binary_file.c_str());
		...
		// Kernel.
        clKernel_in  = clCreateKernel(clProgram, "BFS_in", &clStatus);
		clKernel_0   = clCreateKernel(clProgram, "BFS_0", &clStatus);
        CL_ERR();
    }
