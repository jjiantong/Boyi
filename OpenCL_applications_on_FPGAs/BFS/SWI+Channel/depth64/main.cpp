...
int main(int argc, char **argv) {
    ...
    clStatus =
        clEnqueueWriteBuffer(ocl.clCommandQueue_in, d_nodes, CL_TRUE, 0, sizeof(Node) * n_nodes, h_nodes, 0, NULL, NULL);
    clStatus =
        clEnqueueWriteBuffer(ocl.clCommandQueue_in, d_edges, CL_TRUE, 0, sizeof(Edge) * n_edges, h_edges, 0, NULL, NULL);
    ...
    for(int rep = 0; rep < p.n_reps + p.n_warmup; rep++) {
        ...
        while(*h_num_t != 0) {
            ...
            clStatus = clEnqueueWriteBuffer(
                ocl.clCommandQueue_in, d_q1, CL_TRUE, 0, sizeof(int) * n_nodes, h_q1, 0, NULL, NULL);
            clStatus = clEnqueueWriteBuffer(
            ...
            clSetKernelArg(ocl.clKernel_0, 6, sizeof(cl_mem), &d_tail);
            clSetKernelArg(ocl.clKernel_0, 7, sizeof(cl_mem), &d_iter);
            ...
            while(*h_num_t != 0 ) {
                ...
                if(rep >= p.n_warmup)
                    timer.start("Copy To Device");
                clStatus = clEnqueueWriteBuffer(
                    ocl.clCommandQueue_in, d_num_t, CL_TRUE, 0, sizeof(int), h_num_t, 0, NULL, NULL);
