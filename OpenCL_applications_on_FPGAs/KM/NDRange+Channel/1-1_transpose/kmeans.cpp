...
int allocate(int n_points, int n_features, int n_clusters, float **feature)
{
	...
	// Create the program.
	std::string binary_file = getBoardBinaryFile("1-1_transpose", clDeviceID);
	printf("\nUsing AOCX:%s\n",binary_file.c_str());
	...
	// Kernel.
	clKernel_in  = clCreateKernel(clProgram, "kmeans_kernel_0", &clStatus);
    clKernel_0  = clCreateKernel(clProgram, "kmeans_kernel_1", &clStatus);
    CL_ERR();
