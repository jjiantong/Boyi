...
struct OpenCLSetup {
    ...
    OpenCLSetup(int platform, int device) {
		...
		// Create the program.
		std::string binary_file = getBoardBinaryFile("simd4-1-16_cu8-1-1", clDeviceID);
		printf("\nUsing AOCX:%s\n",binary_file.c_str());
		...
		// Kernel.
        clKernel_0 = clCreateKernel(clProgram, "ScanLargeArrays_kernel", &clStatus);
        clKernel_1 = clCreateKernel(clProgram, "prefixSum_kernel", &clStatus);
        clKernel_2 = clCreateKernel(clProgram, "blockAddition_kernel", &clStatus);
        CL_ERR();
    }
