...
int main(int argc, char **argv) {
    ...
    for(int rep = 0; rep < p.n_warmup + p.n_reps; rep++) {
		...
		clSetKernelArg(ocl.clKernel, 0, sizeof(int), &in_size);
        // Kernel launch
        clStatus = clEnqueueTask(
            ocl.clCommandQueue, ocl.clKernel, 0, NULL, NULL);
        CL_ERR();

		clSetKernelArg(ocl.clKernel_out, 0, sizeof(cl_mem), &d_output);
		clSetKernelArg(ocl.clKernel_out, 1, sizeof(int), &in_size);
        // Kernel launch
        clStatus = clEnqueueTask(
            ocl.clCommandQueue_out, ocl.clKernel_out, 0, NULL, NULL);
        CL_ERR();
